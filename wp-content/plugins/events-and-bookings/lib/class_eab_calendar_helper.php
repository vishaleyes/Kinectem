...
class Eab_CalendarTable_EventArchiveCalendar extends Eab_CalendarTable {
	...
	public function set_event_info ($event_tstamps, $current_tstamps, $event_info) {
		...
		$css_classes = $event_info['status_class'];
		$event_permalink = !empty($event_info['blog_id'])? get_blog_permalink($event_info['blog_id'], $event_info['id']): get_permalink($event_info['id']);
		$tstamp = esc_attr(date_i18n("Y-m-d\TH:i:sO", $event_tstamps['start']));
		...
		$this->_data[] = '<a class="test wpmudevevents-calendar-event ' . $css_classes . '" href="' . $event_permalink . '">' .$event_info['title'] .'<span class="wpmudevevents-calendar-event-info">' .
				(

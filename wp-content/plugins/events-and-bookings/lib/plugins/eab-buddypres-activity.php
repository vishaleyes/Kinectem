...
class Eab_BuddyPress_AutoUpdateActivity {
	...
	function dispatch_creation_activity_update ($post_id) {
		...
		if ('any' == $created) {
			$update = sprintf(__('%s <span class="activity-bold">created an event<span>', Eab_EventsHub::TEXT_DOMAIN), $user_link);
		} else if ('group' == $created && $group_id) {
			$group = groups_get_group(array('group_id' => $group_id));
			$group_link = bp_get_group_permalink($group);
			$group_name = bp_get_group_name($group);
			$update = sprintf(__('%s <span class="activity-bold">created an event in</span> <a href="%s">%s</a>', Eab_EventsHub::TEXT_DOMAIN), $user_link, $group_link, $group_name);
		} else if ('pa' == $created && $public_announcement) {
			$update = sprintf(__('%s <span class="activity-bold">created a public announcement</span', Eab_EventsHub::TEXT_DOMAIN), $user_link);
		}

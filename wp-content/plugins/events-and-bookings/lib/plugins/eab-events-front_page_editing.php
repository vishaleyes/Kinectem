...
/* ----- Add/Edit Links ----- */
	...
	function my_events_add_event () {
		...
		bp_core_new_subnav_item(array(
			'name' => __('Create Event', Eab_EventsHub::TEXT_DOMAIN),
			'slug' => 'edit-event',
	...
	function add_new_link () {
		...
		if (groups_is_user_member($current_user->ID, $current_group)){
		echo '' .
			'<p class="current_groupID">' .
				'<a current_group="'.$current_group.'" href="' . $this->_get_front_editor_link() . '/?current_group='.$current_group.'">' .
					__('Create event', Eab_EventsHub::TEXT_DOMAIN) .
				'</a>' .
			'</p>' .
		'';
		}
...
/* ----- Output ----- */
	...
	private function _edit_event_form ($event_id) {
		...
		$ret .= '<div id="eab-events-fpe" class="createEventForm">';
		...
		$ret .= '<div class="eventTitle createFieldBlock">';
		$ret .= '<label>' . __('Event Title', Eab_EventsHub::TEXT_DOMAIN) . '</label>';
		$ret .= '<div class="createRightBox"><input type="text" name="" id="eab-events-fpe-event_title" value="' . esc_attr($event->get_title()) . '" required="required"/></div>';
		$ret .= '<div class="clearfix"></div></div>';

		$ret .= '<div id="fpe-editor" class="eventEditorWrap clearfix"><div class="clearfix"></div></div>';
	...
	private function _get_event_meta_boxes ($event) {
		$ret = '<div id="eab-events-fpe-meta_info" class="eventMetaInfo">';
		...
		$ret .= '<div class="start-events-fpe-date createFieldBlock">';
		$ret .= '<label>' . __('Starts on', Eab_EventsHub::TEXT_DOMAIN) . '</label>';
		$ret .= '<div class="datebox"> <input type="text" name="" id="eab-events-fpe-start_date" value="' . date('Y-m-d', $start) . '" size="10" />';
		$ret .= ' <input type="text" name="" id="eab-events-fpe-start_time" value="' . date('h:i A', $start) . '" size="6" />';
		$ret .= '</div><div class="clearfix"></div></div>';

		// End date/time
		/*$end = $event->get_end_timestamp();
		$end = $end ? $end : eab_current_time() + 3600;
		$ret .= '<div class="end-events-fpe-date createFieldBlock">';
		$ret .= '<label>' . __('Ends on', Eab_EventsHub::TEXT_DOMAIN) . '</label>';
		$ret .= '<div class="datebox"> <input type="text" name="" id="eab-events-fpe-end_date" value="' . date('Y-m-d', $end) . '" size="10" />';
		...
		// Status
		$ret .= '<div class="createFieldBlock">';
		$ret .= '<label>' . __('Event status', Eab_EventsHub::TEXT_DOMAIN) . '</label>';
		$ret .= '<div class="createRightBox"><select name="" id="eab-events-fpe-status">';
		...
		// Venue
		$ret .= '<div class="createFieldBlock">';
		$ret .= '<label>' . __('Address', Eab_EventsHub::TEXT_DOMAIN) . '</label>';
		$ret .= '<div class="createRightBox"><input type="text" name="" id="eab-events-fpe-venue" value="' . esc_attr($event->get_venue_location()) . '" /></div>';
		$ret .= '<div class="clearfix"></div></div>';
		// End venue
		$ret .= '</div>';
		...
		// Start Duration
		$ret .= '<div class="eab-events-fpe-meta_box" id="eab-events-fpe-meta_box-duration">';
		// Duration
		$ret .= '<div class="eventDuration createFieldBlock">';
		$ret .= '<label>' . __('Duration', Eab_EventsHub::TEXT_DOMAIN) . '</label>';
		$ret .= '<div class="createRightBox"><input type="text" name="duration" id="eab-events-fpe-duration" size="3" maxlength="5" value="' . esc_attr($event->get_duration()) . '" /></div><div class="clearfix"></div>';
		$ret .= '</div>';
		// End Duration
		$ret .= '</div>';
		...
		// Start Reoccurance
		$ret .= '<div class="eab-events-fpe-meta_box" id="eab-events-fpe-meta_box-reoccurance">';
		// Reoccurance
		$ret .= '<div class="createFieldBlock">';
		$ret .= '<label>' . __('Reoccurance', Eab_EventsHub::TEXT_DOMAIN) . '</label>';
		$ret .= '<div class="createRightBox"><select name="reoccurance" id="eab-events-fpe-reoccurance" />';
		$ret .= '<option value="' . esc_attr($event->get_reoccurance()) . '" selected="selected">'. esc_attr($event->get_reoccurance()) .'</option>';
		$ret .= '<option value="Weekly">'.__('Weekly', Eab_EventsHub::TEXT_DOMAIN).'</option>';
		$ret .= '<option value="Monthly">'.__('Monthly', Eab_EventsHub::TEXT_DOMAIN).'</option>';
		$ret .= '<option value="Daily">'.__('Daily', Eab_EventsHub::TEXT_DOMAIN).'</option>';
		$ret .= '</select></div><div class="clearfix"></div>';
		$ret .= '</div>';
		// End Reoccurance
		$ret .= '</div>';

		$ret .= '</div>'; // eab-events-fpe-col_wrapperF

		/* --------- Notes ------------*/
		$ret .= '<div class="eab-events-fpe-col_wrapper">';

		// Start Notes
		$ret .= '<div class="eab-events-fpe-meta_box" id="eab-events-fpe-meta_box-notes">';
		// Notes
		$ret .= '<div class="createFieldBlock">';
		$ret .= '<label>' . __('Notes', Eab_EventsHub::TEXT_DOMAIN) . '</label>';
		$ret .= '<div class="createRightBox"><textarea name="notes" id="eab-events-fpe-notes" value="' . esc_attr($event->get_notes()) . '">'. esc_attr($event->get_notes()) .'</textarea></div><div class="clearfix"></div>';
		$ret .= '</div>';
		// End Notes
		$ret .= '</div>';
		...
		// Categories
		$ret .= '<div  class="createFieldBlock">';
		$ret .= '<label>' . __('Event Type', Eab_EventsHub::TEXT_DOMAIN) . '</label>';
		$ret .= '<div class="createRightBox categories"><select id="eab-events-fpe-categories"><option value=""></option>';
		...
		// Start Opponent
		$ret .= '<div class="eab-events-fpe-meta_box" id="eab-events-fpe-meta_box-opponent">';
		// Opponent
		$ret .= '<div class="createFieldBlock opp-display">';
		$ret .= '<label>' . __('Opponent', Eab_EventsHub::TEXT_DOMAIN) . '</label>';
		$ret .= '<div class="createRightBox"><input type="text" name="opponent" id="eab-events-fpe-opponent" value="' . esc_attr($event->get_opponent()) . '" /></div><div class="clearfix"></div>';
		$ret .= '</div>';
		// End Opponent
		$ret .= '</div>';
		...
		if ($addons) {
			$ret .= '<div class="eab-events-fpe-col_wrapper"><div  class="createFieldBlock">';
			$ret .= $addons;
			$ret .= '</div></div>'; // eab-events-fpe-col_wrapper
		}
	...
	/*	$featured_image = $event->get_featured_image_url();
		...
		if (current_user_can('upload_files')) {
			...
			$ret .= '<div class="eab-events-fpe-col_wrapper createFieldBlock">';
				$ret .= '<label>' . __('Feature Image', Eab_EventsHub::TEXT_DOMAIN) . '</label>' .
					'' .
					'<div class="createRightBox"><a href="#featured_image" class="eab-fpe-upload">' .

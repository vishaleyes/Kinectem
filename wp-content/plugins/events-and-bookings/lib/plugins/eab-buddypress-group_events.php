...
<label for="<?php echo $widget->get_field_id('show_bp_group'); ?>" style="display:block;">
	...
	function filter_query ($query) {
		...
		$posts = array();
		/* ----------- Custom Code ------------*/
		$get_follow_groupID = array();
		$user_ID = get_current_user_id();
		$table_name = $wpdb->prefix . 'bp_team_follow';
		$sql = "SELECT * FROM $table_name WHERE author_id = $user_ID";
		$result = $wpdb->get_results($sql);
    	foreach( $result as $results ) {
        $get_follow_group = $results->group_id;
        $get_follow_groupID[]=$get_follow_group;
    	}
    	$currentGroupID  = $bp->groups->current_group->id;
     	
    	if(!empty($get_follow_groupID) && in_array($currentGroupID, $get_follow_groupID)){
    	$currentGroupID  = $bp->groups->current_group->id;
		...
		foreach ($query->posts as $post) {
			...
			if ($group) {
				...
				if (groups_is_user_member($current_user->ID, $group)){
					$posts[] = $post;
				}
			}
		}
		if (!groups_is_user_member($current_user->ID, $currentGroupID)){
					$posts[] = $postsdata;
				}
		$query->posts = $posts;
...
<?php
	...
	private function _save_meta ($post_id, $request) {
		...
		if( isset( $email_grp_member ) &&  $email_grp_member == 1 ) {
			...
			foreach( $grp_members['members'] as $member ){
				...
				$res= wp_mail( $member->user_email, $subject, $message );
	...
	function add_tab () {
		...
		$name = __('Team Events', Eab_EventsHub::TEXT_DOMAIN);
    	...
    	$currentGroup  = $bp->groups->current_group->id;
    	if(!empty($get_follow_groupID) && in_array($currentGroup, $get_follow_groupID)){
    		$currentGroup  = $bp->groups->current_group->id;
    	
		if($bp->groups->current_group->id==$currentGroup){
		/*if($bp->groups->current_group->is_member!=1)
				$bp->groups->current_group->is_member=1;

		if($bp->groups->current_group->is_visible!=1)
				$bp->groups->current_group->is_visible=1;	

		if($bp->groups->current_group->is_user_member!=1)
				$bp->groups->current_group->is_user_member=1;*/	
		
		if($bp->groups->current_group->user_has_access!=1)

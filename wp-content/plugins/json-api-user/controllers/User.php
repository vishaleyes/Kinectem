    ...
    public function retrieve_password()
    {
        ...
        if (strpos($user_login, '@')) {
            ...
            if (empty($user_data)) {
                ...
                echo json_encode(array('status' => 'error', 'message' => "Your email address not found! ", 'data' => array()));
                exit;
            }
...
//echo $recieverdata['driverID'];
    ...
    {
        if (!$_REQUEST['user_id']) {
            $oReturn->error = __('user_id is required.', 'aheadzen');
            return $oReturn;
        }
        $conn = new mysqli(DB_HOST, DB_USER, DB_PASSWORD, DB_NAME);
        // Check connection
    ...
    public function search2()
    {
        ...
        if (!$_REQUEST['search_term']) {
            $oReturn->error = __('search_term is required.', 'aheadzen');
            return $oReturn;
        }
    ...
    function updateDeviceToken()
    {
        ...
        if (!$_REQUEST['device_type']) {
            $oReturn->error = __('device_type is required.', 'aheadzen');
            return $oReturn;
        }
        if (!$_REQUEST['device_token']) {
            $oReturn->error = __('device_token is required.', 'aheadzen');
            return $oReturn;
        }
        ...
        if (isset($_REQUEST['device_type']) && $_REQUEST['device_type'] == 1) { // device is Android
            $endpointArn = $this->createEndPointForSNS($_REQUEST['device_token'], ANDRIOD_ARN);
            $postdata['device_type'] = $_REQUEST['device_type'];
            $postdata['endpointArn'] = $endpointArn;
        } else if (isset($_REQUEST['device_type']) && $_REQUEST['device_type'] == 2) {// device is ios / iphone

            $endpointArn = $this->createEndPointForSNS($_REQUEST['device_token'], IOS_ARN);
        ...
        $sql = "UPDATE wp_users SET device_token = '" . $endpointArn . "', device_type='" . $_REQUEST['device_type'] . "' WHERE ID = " . $_REQUEST['user_id'];
        $res2 = $conn->query($sql);

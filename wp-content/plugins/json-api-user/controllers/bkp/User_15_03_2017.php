...
public function generate_auth_cookie() {
		...
		if (!$json_api->query->username && !$json_api->query->email) {
			...
			echo json_encode(array('status' => 'error', 'message' => "You must include 'username' or 'email' var in your request to generate cookie.", 'data' => array()));
    exit;
        ...
        if (!empty($token) && !empty($platform_AppArn)) {
            try {
                ...
                try {
                    ...
                    $this->response(array("status" => '-6', "message" => $e->getMessage(), 'data' => array()));
                    exit;
                }
                ...
                $this->response(array("status" => '-3', "message" => "Error while generating endpoint: " . $e->getMessage(), 'data' => array()));
                exit;
            }
        } else {
            $this->response(array("status" => '-4', "message" => "deviceToken is not found. ", 'data' => array()));
            exit;
        }
    }

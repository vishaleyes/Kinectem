...
public function profile_get_settings() {
        ...
        $oReturn->settings->profileInactive = false;
        $oReturn->settings->inMemberDirectory = false;
        $oReturn->settings->inMemberSearch = false;
        ...
        if(get_user_meta($user_id,'_is_account_inactive',true)){$oReturn->settings->profileInactive = true;}
        if(get_user_meta($user_id,'bp_exclude_in_dir',true)=='yes'){$oReturn->settings->inMemberDirectory = true;}
        if(get_user_meta($user_id,'bp_exclude_in_search',true)=='yes'){$oReturn->settings->inMemberSearch = true;}
    ...
    if(get_user_meta($user_id,'notification_groups_email_send','yes') == "yes"){ $oReturn->settings->notification_groups_email_send = true;}

   /* if ( !get_user_meta( $user_id, 'notification_groups_email_send', true) || 'yes' == get_user_meta( $user_id, 'notification_groups_email_send', true) ) {
        $oReturn->settings->notification_groups_email_send = true;
    }*/
        return $oReturn;
    }
...
public function profile_save_settings() {
	...
	if($settingstype=='privacy'){
		...
		if ($_POST['notification_follower_email_send']=='true') {bp_update_user_meta($user_id,'notification_follower_email_send','yes');}
		if ($_POST['notification_follower_email_send']=='false') {bp_update_user_meta($user_id,'notification_follower_email_send','no');}
        if ($_POST['notification_groups_email_send']=='true') {bp_update_user_meta($user_id,'notification_groups_email_send','yes');}
        if ($_POST['notification_groups_email_send']=='false') {bp_update_user_meta($user_id,'notification_groups_email_send','no');}
...
function messages_new_message(){
	...
	foreach($recipientsArr as $row){
		// Notification data making
			...
			//echo $friendData->device_token;
			//echo "<br/>".$friendData->device_type;
...
public function bbp_api_new_topic_handler() {
  ...
  return $oReturn;
	...
	*/
		...
		if($_GET['data']){
			...
			if($info){
				...
				if(in_array($fileExt,$imageExtArr)){
                    ...
                    /*$res =  ($ret ? $ret : $warning);
                    if(isset($res) && $res == "There has been an error processing your request")
                    {
                        echo json_encode(array(
                            "status" => "false",
                            "error" => "error",
    ...
    function activity_preview_video () {
        if($_POST['data']){
            $_POST['data']=urldecode($_POST['data']);
            $BpfbBinder = new BpfbBinder();
            $res =  $BpfbBinder->ajax_preview_video();
            if(isset($res) && $res = "There has been an error processing your request")
            {
	...
	public function activity_set_bpfb()
	{
		...
		if (!empty($_POST['bpfb_url'])) {
			$bpfb_code = $BpfbCodec->create_link_tag($_POST['bpfb_url'],htmlentities($_POST['title']),$_POST['text'],htmlentities($_POST['image']));
			$activity_type = 'activity_update';
		}
	...
	public function activity_add_edit()
	{
		...
		*/
		...
		if($_POST['bpfb_type']=='groups'){
			...
			$bp = buddypress();
			$bp->groups->current_group = groups_get_group( array( 'group_id' => $groupid ) );
			$action  = sprintf( __( '%1$s <span class=\"activity-bold\">posted an update in the group</span> %2$s', 'buddypress'), bp_core_get_userlink( $user_id ), '<a href="' . bp_get_group_permalink( $bp->groups->current_group ) . '">' . esc_attr( $bp->groups->current_group->name ) . '</a>' );
	...
	************************************************/
    ...
    public function friends_get_friends() {
		...
		foreach ($aFriends as $sFriendID) {
			...
			$oReturn->friends [(int) $sFriendID]->user_nicename = $oUser->data->user_nicename;
            $oReturn->friends [(int) $sFriendID]->firstname = xprofile_get_field_data( 1, $sFriendID );
            $oReturn->friends [(int) $sFriendID]->lastname = xprofile_get_field_data( 25, $sFriendID );
			$avatar = bp_core_fetch_avatar( array(
				'object'  => 'user',
				'item_id' => $sFriendID,
				'html'    => false,
    ...
    public function groups_get_group_admins() {
        ...
        foreach ($aGroupAdmins as $oGroupAdmin) {
            ...
            $oReturn->group_admins[(int) $oGroupAdmin->user_id]->is_admin = 1 ;

            /*echo $sql = "select group_id,is_mod, is_admin, user_title,is_confirmed, is_banned, invite_sent  from wp_bp_groups_members where group_id = ".$this->groupid." AND user_id = ".$oGroupAdmin->user_id."";
            $conn->query($sql);
            ...
            $oReturn->group_admins[(int) $oGroupAdmin->user_id]->is_banned  = $memberRecord['is_banned'];
            $oReturn->group_admins[(int) $oGroupAdmin->user_id]->is_admin  = $memberRecord['is_admin'];
            $oReturn->group_admins[(int) $oGroupAdmin->user_id]->is_mod  = $memberRecord['is_mod'];
            $oReturn->group_admins[(int) $oGroupAdmin->user_id]->user_title  = $memberRecord['user_title'];*/
        }
        $aGroupMods = groups_get_group_mods($this->groupid);
        foreach ($aGroupMods as $oGroupMods) {
            $oUser = get_user_by('id', $oGroupMods->user_id);
            $oReturn->group_admins[(int) $oGroupMods->user_id]->username = $oUser->data->user_login;
            $oReturn->group_admins[(int) $oGroupMods->user_id]->mail = $oUser->data->user_email;
            $oReturn->group_admins[(int) $oGroupMods->user_id]->display_name = bpaz_user_name_from_email($oUser->data->display_name);
            $useravatar_url2 = bp_core_fetch_avatar(array('object'=>'member','item_id'=>$oGroupMods->user_id, 'html'=>false, 'type'=>'full'));
            $oReturn->group_admins[(int) $oGroupMods->user_id]->thumb = $useravatar_url2 ;
            $oReturn->group_admins[(int) $oGroupAdmin->user_id]->is_mod = 1 ;
            /*echo $sql = "select group_id,is_mod, is_admin, user_title,is_confirmed, is_banned, invite_sent  from wp_bp_groups_members where group_id = ".$this->groupid." AND user_id = ".$oGroupMods->user_id."";
            $conn->query($sql);
            $resource = $conn->query($sql);
            if ($resource->num_rows > 0) {
                $memberRecord = $resource->fetch_assoc();
            ...
            $oReturn->group_admins[(int) $oGroupMods->user_id]->is_banned  = $memberRecord['is_banned'];
            $oReturn->group_admins[(int) $oGroupMods->user_id]->is_admin  = $memberRecord['is_admin'];
            $oReturn->group_admins[(int) $oGroupMods->user_id]->is_mod  = $memberRecord['is_mod'];
            $oReturn->group_admins[(int) $oGroupMods->user_id]->user_title  = $memberRecord['user_title'];*/
        }

        $oReturn->count = count($aGroupAdmins) + count($aGroupMods);
        return $oReturn;
    }
		...
		*/
		...
		global $members_template;
			...
			foreach($members->results as $row){
				...
				$memberRecord = 0;
				$sql = "select group_id,is_mod, is_admin, user_title,is_confirmed, is_banned, invite_sent  from wp_bp_groups_members where group_id = ".$_GET['groupid']." AND user_id = ".$aMember->ID."";
				$conn->query($sql);
				$resource = $conn->query($sql);
				if ($resource->num_rows > 0) {
					$memberRecord = $resource->fetch_assoc();
                ...
                $oReturn->group_members[$counter]->is_admin  = $memberRecord['is_admin'];
                $oReturn->group_members[$counter]->is_mod  = $memberRecord['is_mod'];
                $oReturn->group_members[$counter]->user_title  = $memberRecord['user_title'];
	...
	************************************************/
	...
	public function accept_friend_request()
	{
		...
		$sql = "SELECT * FROM `wp_bp_friends` WHERE (
	...
	function add_favorite_activity()
	{
		...
		if ( in_array( $activity_id, $my_favs ) ) {
			...
			$oReturn = new stdClass();
			$oReturn->msg = "You have already favorited this activity.";
	...
	function remove_favorite_activity()
	{
		...
		if ( ! isset( $my_favs[ $activity_id ] ) ) {
			$oReturn = new stdClass();
			$oReturn->msg = "You have not allowed to remove unfavorited post.";
			$oReturn->success = 'false';
			return $oReturn;
		}
...
VALUES ('".$_POST['user_id']."','groups', 'created_group', '".$activity_link."','','".$member_link."','".$last_id."',0,'".date("Y-m-d H:i:s")."');";
	...
	foreach($user_ids as $user_id){
		...
		if (empty( $group_id ) ){
			$oReturn = new stdClass();
				$oReturn->error = 'group_id parameter is missing.';
		...
		// Otherwise, create a new invitation
			...
			if ( !$invite->save() ){
				$oReturn = new stdClass();
				$oReturn->error = 'Problem in creating new invitation. Please try again.';
				$oReturn->status = 'false';
				return $oReturn;
			}
		...
		$invited_user_data = bp_core_get_core_userdata( $invited_user_id );
		...
		/*if ( 'no' === bp_get_user_meta( $inviter_id, 'notification_groups_invite', true ) ) {
			$oReturn = new stdClass();
				$oReturn->error = 'Member opted out of receiving this email.';
				$oReturn->status = 'false';
				return $oReturn;
		}*/
		...
		$to       = $invited_user_data->user_email;
        ...
        //echo "<br/>email : ". $to;
		//echo $message;
		//echo "<br/> WP MAIL : ". bp_get_user_meta( $invited_user_id, 'notification_groups_invite', true );
		//die;
		if ( 'yes' === bp_get_user_meta( $invited_user_id, 'notification_groups_invite', true ) ) {
	...
	public function update_team()
	{
		...
		$sql = "UPDATE wp_bp_groups SET $name $slug $description  $location_state  $location_city $status $sport";
		$sql = rtrim($sql, " ,");
...
VALUES ('".$_POST['user_id']."','groups', 'updated_group', '".$activity_link."','','".$member_link."','".$last_id."',0,'".date("Y-m-d H:i:s")."');";
            ...
            groups_notification_group_updated( $_POST['group_id'], $old_group );
	...
	public function delete_team()
	{
		...
		if ($conn->query($sql) === TRUE) {
			...
			$sql = "DELETE FROM `wp_bp_activity` WHERE component = 'groups' AND item_id = ".$_POST['group_id'];
			$res = $conn->query($sql);
			
			$sql = "SELECT * FROM wp_bp_activity WHERE component = 'groups' AND item_id = ".$_POST['group_id']." AND user_id = ".$_POST['user_id'];
			$res = $conn->query($sql);

            $sql = "SELECT * FROM wp_bp_activity WHERE component = 'groups' AND user_id = ".$_POST['user_id'];
            $res = $conn->query($sql);
			if ($res->num_rows > 0) {
			  while($row = $res->fetch_assoc())
			  {
				  $arg['user_id'] = $row['user_id'];
				  $arg['item_id'] = $row['item_id'];
				  bp_activity_delete($arg);
			  }
			}
...
//'<a href=\"http://52.35.93.122/kinectem/members/\">jack</a> changed their profile picture. <br /><img src=\"http://52.35.93.122/kinectem/wp-content/uploads/avatars/72/3bc542520d57470ac10480dad6931b99-bpthumb.jpg\" alt=\"\" />changed their profile picture.';
			...
			if($oActivity->action=='' && $oActivity->content==''){
				$arr['action']  = '`s profile was updated';
			}
		}
		else {
			$arr['action'] = trim($row['action']);
		...
		if(isset($favData) && !empty($favData)){
			if (in_array($row['id'],$favData)) {
				$favoriteCount = $res->fetch_assoc();
				$arr['favorite'] = true;
				$arr['favorite_count'] = (integer)$favoriteCount['meta_value'];
			}
			else
			{
				$favoriteCount = $res->fetch_assoc();
...
//'<a href=\"http://52.35.93.122/kinectem/members/\">jack</a> changed their profile picture. <br /><img src=\"http://52.35.93.122/kinectem/wp-content/uploads/avatars/72/3bc542520d57470ac10480dad6931b99-bpthumb.jpg\" alt=\"\" />changed their profile picture.';
  ...
  ELSE 'replied' END AS comment_type  FROM wp_bp_activity WHERE `type` = 'activity_comment' AND item_id = ".$id." ORDER BY date_recorded ASC";
	...
	function get_my_teams()
	{
		...
		'show_hidden'       => $visibility,    // Show hidden groups to non-admins
	...
	function search_team()
	{
		...
		{
		    if($type == "my"){
                $condition = "WHERE `name` LIKE '%" . $_REQUEST['keyword'] . "%' and status != 'Hidden' AND wg.`creator_id` = ".$_REQUEST['user_id'];
            } else {
...
function follow_team()
{
    ...
    $sql = "SELECT * FROM `wp_bp_groups_members` WHERE user_id = ".$_REQUEST['author_id']." AND group_id = ".$_REQUEST['group_id'];
    $res = $conn->query($sql);
    if ($res->num_rows > 0)
        $groupMembers = $res->fetch_assoc();
    if(isset($groupMembers['is_banned']) && $groupMembers['is_banned'] == 1)
    {
        $oReturn = new stdClass();
        $oReturn->error = 'error';
        $oReturn->status = 'ok';
	...
	$friendData = $friendDataObj->data;
	$friendData->notification_type_id = 6;
    $group = groups_get_group( array( 'group_id' => (int) $_REQUEST['group_id'] ) );
    $friendData->groupData = $group;
  ...
  WHERE meta_key = 'starred_by_user'
   ...
   AND wr.`is_deleted` = 0 AND wr.`user_id` = ".$_REQUEST['user_id']."";
	...
	if ($res->num_rows > 0) {
			 $i=0;
			 while($row = $res->fetch_assoc()) {
			 	$useravatar_url = bp_core_fetch_avatar(array('object'=>'member','item_id'=>$row['sender_id'], 'html'=>false, 'type'=>'full'));
				$arr[$i] = $row;
...
WHERE wr.user_id != ".$row['sender_id']." AND wr.thread_id = ".$row['thread_id']."";
				$res2 = $conn->query($sql);
...
WHERE wm.sender_id = ".$_REQUEST['user_id']." AND wr.`is_deleted` = 0 AND wr.`user_id` = ".$_REQUEST['user_id']." GROUP BY thread_id ORDER BY id DESC";
	$res = $conn->query($sql);
  ...
  ', 'followeremail' ), $email_text, get_blog_option( BP_ROOT_BLOG, 'blogname' ), stripslashes( esc_attr( $bp->groups->current_group->name ) ), $group_link );
	...
	*/
	function groups_promote_member()
	{
        $oReturn = new stdClass();
		if(!$_REQUEST['user_id']){$oReturn->error = __('user_id is required.','aheadzen'); return $oReturn;}
        //if(!$_REQUEST['friend_id']){$oReturn->error = __('friend_id is required.','aheadzen'); return $oReturn;}
		if(!$_REQUEST['group_id']){$oReturn->error = __('group_id is required.','aheadzen'); return $oReturn;}
		...
		try{
			...
			$friendDataObj = get_userdata($_REQUEST['friend_id']);
			$userData = $userDataObj->data;
			$friendData = $friendDataObj->data;
			$friendData->notification_type_id = 9;

			$message = $userData->display_name. " have promoted you in his group ".$groupData['name'];
			$this->sendPushNotification($message, $friendData->device_token, $userData, $friendData->device_type, $friendData);

            groups_notification_promoted_member($_REQUEST['user_id'],$_REQUEST['group_id']);
			$oReturn = new stdClass();
...
WHERE wr.user_id != ".$row2['sender_id']." AND wr.thread_id = ".$_REQUEST['thread_id']."";
	...
	public function getGroupInvitListByUser()
	{
	    ...
	    while($row2 = $res2->fetch_assoc())
	    {
				...
				$arr[$j]['userData'] = $user->data;
				$arr[$j]['groupData'] = $groupData ;
				$j++;
				}
				$i++;
	    }
	...
	public function search()
	{
			...
			$this->do_search( $args );
			//print "<pre>";print_r($this->search_results);die;
			...
			if( isset( $this->search_results['all']['items'] ) && !empty( $this->search_results['all']['items'] ) ){
				...
				//print "<pre>"; print_r($search_results);die;
			...
			//$search_results[]['total_match_count'] = $this->search_results['all']['total_match_count'];
			die( json_encode( array("status"=>"ok", "success"=>"success", "data"=>$search_results,"total_match_count" => $this->search_results['all']['total_match_count']) ) );
...
SELECT * FROM `wp_bp_friends` WHERE friend_user_id = ".$_REQUEST['user_id']." AND is_confirmed = 1";
   ...
   function recount_bookings($event_id) {
	...
	function accept_membership_request()
	{
        ...
        $user_data = $user->data;
        $user_data->notification_type_id = 4;
        $group = groups_get_group( array( 'group_id' => (int) $_REQUEST['group_id'] ) );
        $user_data->groupData = $group;
        ...
        $message = $user_data->display_name. " accepted your membership request.";
        $this->sendPushNotification($message, $user_data->device_token, $groupData, $user_data->device_type, $user_data);
	...
	public function get_team_by_id()
	{
		...
		if ($res->num_rows > 0) {
			...
			while($row = $res->fetch_assoc()) {
				...
				$groupData['user_avatar'] = $useravatar_url;
			}
		}

		$groupavatar_url = bp_core_fetch_avatar(array('object'=>'group','item_id'=>$groupData['id'], 'html'=>false, 'type'=>'full'));
		$groupData['avatar'] = $groupavatar_url;
		...
		if($groupData['creator_id'] == $_REQUEST['user_id'])
			...
			$oReturn->is_invited = groups_check_user_has_invite((int) $_REQUEST['user_id'], $groupData['id'], $this->type);
			$groupData['is_invited'] = is_null($oReturn->is_invited) ? 0 : 1;

			$oReturn->membership_requested = groups_check_for_membership_request((int) $_REQUEST['user_id'], $groupData['id']);
	...
	public function searchAll()
  {
    ...
    $sql = "(SELECT DISTINCT
              ...
              entry_date DESC ";
			$limitQuery = "limit $start, $end";
			$query = 		$sql.' '.$limitQuery;
			$queryCount = $sql;

      $response = array();
      $res = $conn->query($query);
	  $res_count = $conn->query($queryCount);
      ...
      if ($res->num_rows > 0) {
          while($row2 = $res->fetch_assoc())
          {
              ...
              if($row2['TYPE'] == "groups")
              {
                 ...
                 $userData = $this->getGroupByID($row2['id']);
				 $userData['TYPE'] =  'groups';
				 $response[] = $userData;
              ...
              {
                $memberData = get_userdata($row2['id']);
				$memberData->data->TYPE = 'members';
                ...
                $user = new BP_Core_User( $row2['id'] );
          			 if($user->avatar){
						...
						$res->cover_pic = $bbp_cover_pic;
          				...
          				$memberData->data->avatar = $user_avatar_src;
          				$memberData->data->avatar_big = $user_avatar_src;
          				$memberData->data->avatar_thumb = $avatar_thumb_src;
          				$memberData->data->avatar_mini = $avatar_mini;

               }
                ///$res2 = bp_get_user_meta($row['id']);
								$response[] = $memberData->data;
              }
	        ...
	        $oReturn->success = "success";
	        $oReturn->data = $response;
			$oReturn->total_records = $total_records;
	        return $oReturn;
      }
	  else{
			$oReturn = new stdClass();
	        $oReturn->status = "1";
	        $oReturn->success = "success";
			$oReturn->message = "No data found.";
	        $oReturn->data = array();
			$oReturn->total_records = 0;
	        return $oReturn;
	  }
  ...
  function getGroupByID($groupID=NULL)
  {
	...
	$groupavatar_url = bp_core_fetch_avatar(array('object'=>'group','item_id'=>$groupID, 'html'=>false, 'type'=>'full'));
	$groupData['group_avatar'] = $groupavatar_url;
	...
	{
			$data = bp_activity_get_user_favorites(131);
			print_r($data);die;
            ...
            try {
               ...
               // if ($reciever_user_type == 1) { // User is Driver
                    if ($recieverdata->device_type == 1) {  // device is andriod
                        ...
                        $msgpayload = json_encode(array('data' => array('message' => $push_message, 'sound' => $sound, $senderdata, 'notification_type_id' => $recieverdata->notification_type_id, 'receiverData' => $recieverdata, 'senderData' => $senderdata)));
                        ...
                        if(!isset($recieverdata->notification_type_id))
                        {
                            $recieverdata->notification_type_id = 0;
                        }
                        $msgpayload = json_encode(array('aps' => array('alert' => $push_message, 'sound' => $sound, $senderdata, 'notification_type_id' => $recieverdata->notification_type_id, 'receiverData' => $recieverdata, 'senderData' => $senderdata)));

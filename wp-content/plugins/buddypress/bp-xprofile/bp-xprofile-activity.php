...
function bp_xprofile_format_activity_action_new_avatar( $action, $activity ) {
	...
	$action   = sprintf( __( '%s <span class="activity-bold">changed their profile picture</span>', 'buddypress' ), $userlink );
...
function bp_xprofile_format_activity_action_updated_profile( $action, $activity ) {
	...
	$action	      = sprintf( __( "%s <span class='activity-bold'>'s profile was updated </span>", 'buddypress' ), '<a href="' . $profile_link . '">' . bp_core_get_user_displayname( $activity->user_id ) . '</a>' );

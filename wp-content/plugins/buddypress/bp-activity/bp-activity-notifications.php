...
', 'buddypress'), $poster_name, $content, $message_link);
        ...
        $sql = "SELECT * FROM wp_bp_activity WHERE id = '".$activity_id."'";
        $res = $conn->query($sql);
        $activityData = $res->fetch_assoc();
        $senderUser = bp_core_get_core_userdata($activityData['user_id']);

        //print_r($senderUser); die;
        $push_message = $message;
        $endpointArn = $ud->device_token;
        $senderdata = $senderUser;
        $reciever_user_type = $ud->device_type;
		$recieverdata = $ud;
		$recieverdata->notification_id = 5;
        $res = sendPushNotification($push_message, $endpointArn, $senderdata, $reciever_user_type ,$recieverdata);
    ...
    if (!empty($push_message) && !empty($endpointArn)) {
        ...
        try {
            ...
            if ($recieverdata->device_type == 1) {  // device is andriod
                ...
                $msgpayload = json_encode(array('data' => array('message' => $push_message, 'sound' => $sound, $senderdata, 'notification_type_id' => $recieverdata->notification_id, 'receiverData' => $recieverdata, 'senderData' => $senderdata)));
                ...
                if(!isset($recieverdata->notification_id))
                {
                    $recieverdata->notification_type_id = 0;
                }
                $msgpayload = json_encode(array('aps' => array('alert' => $push_message, 'sound' => $sound, 'notification_type_id' => $recieverdata->notification_id, 'receiverData' => $recieverdata, 'senderData' => $senderdata)));

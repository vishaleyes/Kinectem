...
function bp_insert_activity_meta( $content = '' ) {
	...
	$new_content = str_replace( '<br/><span class="time-since">%s</span>', '', $content );
	...
	$time_since = apply_filters_ref_array( 'bp_activity_time_since', array(
		'<br/><span class="time-since">' . $date_recorded . '</span>',
		&$activities_template->activity
	) );
	...
	if ( ! bp_is_single_activity() ) {
		...
		$activity_meta      = sprintf( '%1$s <a href="%2$s" class="view activity-time-since" title="%3$s"><br/>%4$s</a>',
			$new_content,
			$activity_permalink,

...
function groups_post_update( $args = '' ) {
	...
	// Record this in activity streams
	$activity_action  = sprintf( __( '%1$s <span class="activity-bold">posted an update in the group</span> %2$s', 'buddypress'), bp_core_get_userlink( $user_id ), '<a href="' . bp_get_group_permalink( $bp->groups->current_group ) . '">' . esc_attr( $bp->groups->current_group->name ) . '</a>' );

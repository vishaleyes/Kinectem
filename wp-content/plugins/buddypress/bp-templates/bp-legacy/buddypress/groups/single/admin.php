...
<?php if ( bp_is_group_admin_screen( 'edit-details' ) ) : ?>
	...
	<label for="group-name"><?php _e( 'Team Name (required)', 'buddypress' ); ?></label>
	<input type="text" name="group-name" id="group-name" value="<?php bp_group_name(); ?>" aria-required="true" />

	<label for="group-desc"><?php _e( 'Team Description (required)', 'buddypress' ); ?></label>
	<textarea name="group-desc" id="group-desc" aria-required="true"><?php bp_group_description_editable(); ?></textarea>
	<label for="group-sports"><?php _e( 'Select Sports', 'buddypress' ); ?></label>
		<select name="group-sports" id="group-sports">
			<option selected="selected" value="<?php bp_group_sport(); ?>"><?php bp_group_sport(); ?></option>
	...
	<label for="group-location"><?php _e( 'Location (required)', 'buddypress' ); ?></label>
	<input type="text" name="group-location-state" id="group-location-state" value="<?php bp_group_location_state(); ?>" aria-required="true" placeholder="State"/>
	<input type="text" name="group-location-city" id="group-location-city" value="<?php bp_group_location_city(); ?>" aria-required="true" placeholder="City"/>
	<?php
	...
	<p>
		<label for="group-notify-members">
			<input type="checkbox" name="group-notify-members" id="group-notify-members" value="1" /> <?php _e( 'Notify team members of these changes via email', 'buddypress' ); ?>
		</label>
	</p>
...
<?php if ( bp_is_group_admin_screen( 'group-settings' ) ) : ?>
	...
	<div class="radio">
		...
		<label for="group-status-public"><input type="radio" name="group-status" id="group-status-public" value="public"<?php if ( 'public' == bp_get_new_group_status() || !bp_get_new_group_status() ) { ?> checked="checked"<?php } ?> aria-describedby="public-group-description" /> <?php _e( 'This is a public team', 'buddypress' ); ?></label>

		<ul id="public-group-description">
			<li><?php _e( 'Any site member can join this team.', 'buddypress' ); ?></li>
			<li><?php _e( 'This team will be listed in the teams directory and in search results.', 'buddypress' ); ?></li>
			<li><?php _e( 'Team content and activity will be visible to any site member.', 'buddypress' ); ?></li>
		</ul>

		<label for="group-status-private"><input type="radio" name="group-status" id="group-status-private" value="private"<?php if ( 'private' == bp_get_new_group_status() ) { ?> checked="checked"<?php } ?> aria-describedby="private-group-description" /> <?php _e( 'This is a private team', 'buddypress' ); ?></label>

		<ul id="private-group-description">
			<li><?php _e( 'Only users who request membership and are accepted can join the team.', 'buddypress' ); ?></li>
			<li><?php _e( 'This team will be listed in the teams directory and in search results.', 'buddypress' ); ?></li>
			<li><?php _e( 'Team content and activity will only be visible to members of the team.', 'buddypress' ); ?></li>
		</ul>

		<label for="group-status-hidden"><input type="radio" name="group-status" id="group-status-hidden" value="hidden"<?php if ( 'hidden' == bp_get_new_group_status() ) { ?> checked="checked"<?php } ?> aria-describedby="hidden-group-description" /> <?php _e('This is a hidden team', 'buddypress' ); ?></label>

		<ul id="hidden-group-description">
			<li><?php _e( 'Only users who are invited can join the team.', 'buddypress' ); ?></li>
			<li><?php _e( 'This team will not be listed in the teams directory or search results.', 'buddypress' ); ?></li>
			<li><?php _e( 'Team content and activity will only be visible to members of the team.', 'buddypress' ); ?></li>
		</ul>
	...
	<h4><?php _e( 'Team Invitations', 'buddypress' ); ?></h4>

	<p><?php _e( 'Which members of this team are allowed to invite others?', 'buddypress' ); ?></p>

	<div class="radio">

		<label for="group-invite-status-members"><input type="radio" name="group-invite-status" id="group-invite-status-members" value="members"<?php bp_group_show_invite_status_setting( 'members' ); ?> /> <?php _e( 'All team members', 'buddypress' ); ?></label>

		<label for="group-invite-status-mods"><input type="radio" name="group-invite-status" id="group-invite-status-mods" value="mods"<?php bp_group_show_invite_status_setting( 'mods' ); ?> /> <?php _e( 'Team admins and mods only', 'buddypress' ); ?></label>

		<label for="group-invite-status-admins"><input type="radio" name="group-invite-status" id="group-invite-status-admins" value="admins"<?php bp_group_show_invite_status_setting( 'admins' ); ?> /> <?php _e( 'Team admins only', 'buddypress' ); ?></label>
...
<?php if ( bp_is_group_admin_screen( 'group-avatar' ) ) : ?>
	...
	<?php if ( 'upload-image' == bp_get_avatar_admin_step() ) : ?>
			...
			<p><?php _e("Upload an image to use as a profile photo for this team. The image will be shown on the main team page, and in search results.", 'buddypress' ); ?></p>
			...
			<?php if ( bp_get_group_has_avatar() ) : ?>
				...
				<p><?php _e( "If you'd like to remove the existing team profile photo but not upload a new one, please use the delete team profile photo button.", 'buddypress' ); ?></p>

				<?php bp_button( array( 'id' => 'delete_group_avatar', 'component' => 'groups', 'wrapper_id' => 'delete-group-avatar-button', 'link_class' => 'edit', 'link_href' => bp_get_group_avatar_delete_link(), 'link_title' => __( 'Delete Team Profile Photo', 'buddypress' ), 'link_text' => __( 'Delete Team Profile Photo', 'buddypress' ) ) ); ?>
...
<?php if ( bp_is_group_admin_screen( 'group-cover-image' ) ) : ?>
	...
	<p><?php _e( 'The Cover Image will be used to customize the header of your team.', 'buddypress' ); ?></p>
	...
	<div class="bp-widget">
					...
					<li class="<?php bp_group_member_css_class(); ?>">
						...
						<h5>
							...
							<?php endif; ?>
								...
								<a href="<?php bp_group_member_remove_link(); ?>" class="button confirm" title="<?php esc_attr_e( 'Remove this member', 'buddypress' ); ?>"><?php _e( 'Remove from team', 'buddypress' ); ?></a>
		...
		<?php else: ?>
			...
			<div id="message" class="info">
				<p><?php _e( 'This team has no members.', 'buddypress' ); ?></p>
			</div>
...
<?php if ( bp_is_group_admin_screen( 'delete-group' ) ) : ?>
	...
	<div id="message" class="info">
		<p><?php _e( 'WARNING: Deleting this team will completely remove ALL content associated with it. There is no way back, please be careful with this option.', 'buddypress' ); ?></p>
	</div>

	<label for="delete-group-understand"><input type="checkbox" name="delete-group-understand" id="delete-group-understand" value="1" onclick="if(this.checked) { document.getElementById('delete-group-button').disabled = ''; } else { document.getElementById('delete-group-button').disabled = 'disabled'; }" /> <?php _e( 'I understand the consequences of deleting this team.', 'buddypress' ); ?></label>
	...
	<div class="submit">
		<input type="submit" disabled="disabled" value="<?php esc_attr_e( 'Delete Team', 'buddypress' ); ?>" id="delete-group-button" name="delete-group-button" />
	</div>

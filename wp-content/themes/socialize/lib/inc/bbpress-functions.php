...
if ( class_exists( 'bbPress' ) ) {
	...
	if ( ! function_exists( 'socialize_bbp_enqueue_styles' ) ) {	
		function socialize_bbp_enqueue_styles() {
			wp_enqueue_style( 'gp-bbp', socialize_css_uri . 'bbpress.css' );
		}
	}
	add_action( 'wp_enqueue_scripts', 'socialize_bbp_enqueue_styles' );

	// New forum layout 
	function socialize_bbp_list_forums( $args = '' ) {
		...
		if ( !empty( $sub_forums ) ) {
			...
			foreach ( $sub_forums as $sub_forum ) {
				...
				if ( ! empty( $show_freshness_link ) ) {
					$freshness_link = "<div class='freshness-forum-link'>" . socialize_get_last_poster_block( $sub_forum->ID ) . "</div>";
				}
	...
	function socialize_last_poster_block( $subforum_id = "" ) {
		echo socialize_get_last_poster_block( $subforum_id = "" );
	}

	function socialize_get_last_poster_block( $subforum_id = "" ) {
